...
OpenCode 2.x 是待适配的独立宿主 API，不由上述下限推导为已支持。适配工作须保留当前 V1 入口及验证基线，使用独立 V2 适配边界；权限规则转换必须保序、保留资源与 effect，并对尚未证明等价的 action 通配模式拒绝转换。转换不构成授权。V2 公开 API 未满足资源级宿主授权、父链、分页来源、状态与原生卡片要求前，不得接通受管执行或降低这些要求。不得自动迁移旧 Task 账本、读取凭据或改写用户 V1 配置。当前阶段证据见 `test/opencode-v2-permissions.test.mjs`；完整 V2 Runtime、CLI/TUI 和真实生命周期仍待实施及验收。

### 独立 V2 原生契约（增量目标，非 V1 等价）

用户允许定义不同于 V1 的 V2 产品边界；本节不修改以上 V1 需求。这里的「原生」仅指复用 OpenCode V2 的 Agent、工具、权限和 Session 机制，不宣称 O4E `task`、`bash`、`o4e_task`、Workflow 或其持久账本已迁移。每一条的目标与实测必须分开陈述；当前只在 Linux / OpenCode 2.0.15 的隔离假模型中验收部分路径，不能据此发布完整 V2 支持声明。

| ID | 独立 V2 目标与验收条件 | 当前证据和仍缺少的条件 |
| --- | --- | --- |
| `V2-NAT-001` | V1 入口和生成物不变；V2 通过独立入口、显式 opt-in 和可归属的生成输出加载。现有项目、全局同名 Agent/配置合并时，不能仅按名称认定所有权或覆盖用户配置；冲突须拒绝或有可验证的隔离策略。 | 独立只读插件/TUI 状态和只向空隔离目录输出的全拒绝 Agent + 守卫通过宿主验证；修改 Agent 的最终规则时模型请求被拒绝，同名配置合并实测仍无工具。仅证明该隔离夹具条件；生产安装、全局来源归属、其他插件交互、重建/卸载仍缺。 |
| `V2-NAT-002` | 如果提供可执行的原生 Agent，必须使用宿主真实资源级 `allow/ask/deny`，明确 CLI `--auto` 与交互审批的差异；O4E 自有副作用不得借原生工具的授权结果自动放行。失败读取、未知来源或无法验证规则时拒绝相应副作用。只读状态工具不构成授权。 | 原生 `shell.execute` 包装的单命令 allow/deny、非交互 ask 拒绝、`--auto` 放行和运行中中断通过隔离验证；生产权限配置、人工审批、审批中取消、复杂 Shell/进程树、退出/重启尚缺。 |
| `V2-NAT-003` | 若宣称原生子 Agent 的**父权限上界**，必须在每次相关原生工具执行前基于可信 `parentID`、父 Agent 身份和实际资源收紧宿主决策；不能只依赖子 Agent 的自有规则、提示词或进程内记忆。无法核实父链、父规则或授权来源时须拒绝；覆盖所有拟支持的工具/资源、祖先链、嵌套、ask/deny、配置变化与失败路径后方可作此声明。否则仅描述为**子 Agent 使用自身权限**，不启用或不推荐跨越父权限的委派。 | 原生子 Session 有物理父链，但默认子 `allow` 可执行父 `deny`。公开 `session.get` + `permission.hook("evaluate")` 在一个 Shell 资源上成功把子 `allow` 改 `deny`；有界父链读取对缺失、循环、越深与查询错误拒绝；隔离实机证明一个直接父节点的 Shell 收紧。查询错误时子任务仍可报告 completed；动态父规则、全工具与错误可见性均未验收，探针不是生产权限继承实现。 |
| `V2-NAT-004` | 原生 foreground/background、状态及取消只按宿主已证明的语义标注；若宣称 O4E 管理的持久后台任务，必须另行证明有界来源、状态去重、原始 Part 更新或明确独立 UI、进程退出与重启恢复、取消及不确定副作用。不得用原生 Session 存在代替账本证明。 | 仅单次运行中的原生 Shell PID 中断通过；后台、恢复、原卡片与管理界面未验收，因此不提供 O4E Task 管理声明。 |
| `V2-NAT-005` | 发布范围按功能逐项标记 `available`、`experimental` 或 `unavailable` 并附真实宿主证据；用户文档不得将预览、实验或原生行为写作 V1 等价。只有安装产物、隔离配置和正常失败路径均验收后才能开放相应默认入口；其他系统单独声明未验收。 | 当前仅可交付只读状态与隔离全拒绝 Agent 预览；原生执行/收紧只属于验收探针。 |

完整 V1 等价仍另设 Gate：V1 受管权限、父链、分页来源、持久状态、原始卡片及生命周期要求继续适用；允许的 V2 原生范围不能反向豁免或悄悄重用 V1 的受管工具名称与状态表述。当前 `test/acceptance/v2-native.mjs` 是真实宿主实验，不是启用生产执行的证据。
...
Command SQLite 默认放在用户数据目录的 `opencode-for-everything/command-ledgers/<directory-sha256>.sqlite`。Linux/macOS 使用绝对 `XDG_DATA_HOME`（缺省 `~/.local/share`），Windows 使用 `LOCALAPPDATA`（缺省 `~/AppData/Local`）。私有目录和数据库必须验证所有者、类型及权限；POSIX 目录 0700、文件 0600。数据库保存完整有界恢复视图，不按日志 24 小时 TTL 删除任务；备份、移动机器或手动删除时必须将该数据库作为会话恢复数据的一部分，不能仅复制 OpenCode 数据库。安装、构建、卸载不迁移或删除这些任务记录。

SQLite 的写入/提交失败仍属于规范持久化不确定，保留现有停止、取消、资源和祖先锁边界。宿主展示摘要发布失败不回滚已确认的 SQLite 记录、不重放执行，查询必须暴露 `O4E_COMMAND_PROJECTION_UNAVAILABLE`，指定 owner 的后续恢复/更新可重新发布并清除诊断；摘要发布不保证界面同帧刷新。此处的宿主摘要失败与下文 CMD-009 的规范持久化失败区分。原生 Shell 卡片仍由原始 Part 更新，刷新间隔、最终输出、截断预算和日志生命周期保持不变。

Session 更新必须先读取最新状态并完成 transform 的权限/CAS 校验；O4E 内容完全相同时跳过宿主更新，不能依赖空更新事件维持生命周期，亦不得缓存授权状态。revision 变化代表新用户意图、取消屏障或真实事务时仍须持久化。
...
| `CMD-007` | 命令任务必须使用独立校验的 `kind: command` 记录，以仓库外 O4E SQLite 中按 directory/owner/task 隔离的记录为唯一规范账本；ref 与记录的 `taskSessionID` 必须等于 `ownerSessionID`，仅标识所有者，不创建执行 Session。源 Session/消息/调用身份用于创建去重；规范记录、claim 与引用在同一 SQLite 事务中校验并提交，仅更新变化的任务行，声明提交确认先于端口执行。owner Session 的 `metadata.o4e.commandTasks` 只保存 version 2 的逐任务白名单 `snapshot` 摘要，不保存 recovery、输出正文、日志路径或完整授权记录。摘要供现有 TUI 显示和来源定位，不构成执行、恢复或 Workflow Gate 授权；这些操作必须读取独立账本。不得覆盖 owner 的 Agent `metadata.o4e.task`、委派信封或其他元数据。命令通道按 owner 遵守 `maxConcurrentCommands`，不消耗 Agent 槽位，不创建写锁。恢复只查询指定 owner，并在检查句柄前恢复不确定资源准入；可连接进程内已有句柄，不得重放旧声明、跨宿主重启接管 PID 或重建未提交命令。未提交记录进入 `interrupted/not-submitted`；已声明但句柄缺失保持 `unknown` 和保守资源槽。独立账本缺失、不可读或当前结构不可验证必须拒绝，不能用展示摘要补建。旧 Session recovery 布局不得自动读取、迁移、删除或伪装为新记录。 | `src/runtime/command-ledger-store.mjs`; `src/runtime/command-task-runtime.mjs`; `test/command-ledger-store.test.mjs`; `test/plugin-command-task.test.mjs` |
...
| `CMD-009` | Inspect/watch 读取器中止或读取截止时间不得取消命令。Bash 返回 queued/running 快照后，中止原读取器不得取消执行。普通根所有者 Session 空闲或根回合的 `MessageAbortedError`/`AbortError` 必须保留已分离命令；根回合 abort 只临时抑制自动跟踪。附着的 `bash` 调用方中止、所有者 Session 删除、显式 Task cancel、受管子任务生命周期终止/取消以及插件释放仍必须发起取消；执行超时必须发起停止。分离不得削弱子任务权限或生命周期边界。停止证据只能覆盖已验证的所属进程组。未知停止或规范 SQLite 持久化失败必须保留 Command 资源准入及不确定记录，不创建 Command 写锁；取消/释放重试可以重试停止证据或持久化，但绝不能重试执行。Agent 父级的停止及保锁要求仍按 Agent 契约执行。不得声称存在跨进程租约、OS 沙箱或能停止通过 `setsid`/`setpgid` 逃逸的后代。 | `src/adapters/opencode/plugin-hooks.ts`; `src/command-core/command-execution-port.mjs`; `src/runtime/command-task-runtime.mjs`; `test/plugin-command-task.test.mjs`; `test/command-task-runtime.test.mjs`; `test/command-execution-port.test.mjs` |
